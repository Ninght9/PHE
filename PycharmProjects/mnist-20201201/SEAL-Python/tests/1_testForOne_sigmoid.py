...
test_loader = DataLoader(dataset=test_data, batch_size=10,pin_memory=True,num_workers=multiprocessing.cpu_count())  #设置pin_memory=True，则意味着生成的Tensor数据最开始是属于内存中的锁页内存，这样将内存的Tensor转义到GPU的显存就会更快一些。http://www.voidcn.com/article/p-fsdktdik-bry.html
