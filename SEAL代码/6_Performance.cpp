    ...
    partial class Examples
    {
        private static void BFVPerformanceTest(SEALContext context)
        {
            ...
            */
            ...
            /*
            Populate a vector of values to batch.填充要批处理的值的矢量。
            */
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                right size so unnecessary reallocations are avoided.这里没有什么不寻常的地方。我们将随机明文矩阵分批成多项式。
				注意，我们创建的明文大小是完全正确的，因此避免了不必要的重新分配
                */
                ...
                /*
                [Unbatching]
                We unbatch what we just batched. 我们把刚批次的东西打开。
                */
                ...
                our random batched matrix here.我们确保已经分配了密文，并且密文足够大，
				可以使用这些加密参数进行加密。 我们在这里加密随机批处理矩阵。
                */
                ...
                */
                ...
                to avoid reallocating during multiplication.我们把两个密文相乘。由于结果的大小为3，
				并且将覆盖第一个参数，因此我们首先保留足够的内存，以避免在乘法过程中重新分配
                */
                ...
                encrypted2 here.我们将密文与随机明文相乘。回想被乘数不改变密文的大小，因此我们在此使用Encrypted2
                */
                ...
                faster than generic homomorphic multiplication.我们继续使用Encrypted2。现在我们平方它，这应该比一般同态的乘法快。
                */
                timeSquareSum.Start();
                ...
                if (context.UsingKeyswitching)
                {
                    ...
                    needed in the process.该回去加密了。我们现在将其重新定位到大小2。由于分配当前足够大，
					可以包含大小为3的密文，因此在此过程中不需要昂贵的重新分配。
                    */
                    ...
                    We rotate matrix rows by one step left and measure the time.我们将矩阵行旋转一步，并测量时间。
                    */
                    ...
                    expensive than rotating by just one step.我们通过随机步骤旋转矩阵行。这比旋转一步要昂贵得多
                    */
        ...
        private static void ExampleBFVPerformanceDefault()  /*默认 */ 
        {
        ...
        private static void ExampleBFVPerformanceCustom()   /*自定义 */ 
        {
